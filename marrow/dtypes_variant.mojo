"""DataType redesign — Variant-based approach (parallel prototype).

The type-erased `AnyType` container uses `Variant` as storage instead of
`ArcPointer` + vtable function pointers.

Trade-offs vs the vtable approach:
- No heap allocation — discriminated union stored inline on the stack.
- No indirection — direct member access, no function-pointer dispatch.
- Closed set — all concrete types must be enumerated in the Variant.
- Slightly larger — size-of-largest-member + 1-byte discriminant.

This module is a *parallel prototype* — it does not replace the existing
`DataType` struct.
"""
...
struct Field(Equatable, ImplicitlyCopyable, Movable, Writable):
    var name: String
...
struct AnyType(Copyable, Equatable, ImplicitlyCopyable, Movable, Writable):
    var _v: _AnyTypeV
