...
def test_add_gpu():
    ...
    var result = add[int32](a, b, ctx).to_host(ctx)
...
def test_add_gpu_float32():
    ...
    var result = add[float32](a, b, ctx).to_host(ctx)
    assert_equal(len(result), 4)
...
def test_device_round_trip():
    ...
    assert_true(on_device.buffer.has_device())

    # Download back to host
    var on_host = on_device.to_host(ctx)
...
def test_chained_gpu_add():
    ...
    assert_true(ab.buffer.has_device())

    # Second add: (a+b) + c → reuses device buffer from ab
    var abc = add[int32](ab, c, ctx).to_host(ctx)
